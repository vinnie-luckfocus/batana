...
| **standard-imu** | 仅 IMU | batana-cap + 手机（cap 本身不算分，圆屏只显示片上粗估） | 棒速/计数近实时（≤ 1s） |
...
- **时间同步前提**：凡涉及 IMU 与视频融合的档位（pro-fusion / max），依赖 ble-protocol 的时钟同步命令（0x05）建立 `clock_anchor`；对齐误差目标 ≤ 2ms，做不到则该次会话降级为粗略融合并在结果中标注。

## 5. 降级与故障语义

| 场景 | 行为 |
|---|---|
| 分析中途 BLE 断连（cap 掉线） | 视觉部分继续完成；结果标记 `partial: true`，IMU 相关指标缺失；会话可保存 |
| 分析中途相机中断 | 仅 IMU 部分保留，按 standard-imu 口径出结果；不假装视觉指标存在 |
| pi 离线 | 本地完成分析与显示，会话入离线队列；联网后按 sync-api 幂等上传（重复 `session_id` 返回 409 + content_hash 比对） |
| 外设热插拔 | 档位在**会话之间**切换；进行中的会话不切档，按开始时的档位跑完 |
| 时钟同步失败 | 见第 4 节：降级粗略融合并标注，不阻塞出分 |
