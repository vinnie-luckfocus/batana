...
```
        ...
        ▼              ▼               ▼               ▼              ▼
  ┌──────────┐  ┌───────────┐  ┌────────────┐  ┌────────────┐  ┌───────────┐
  │batana-pi │  │batana-cap │  │batana-core │  │batana-app  │  │batana-web │
  │双目边缘盒 │  │棒尾传感器  │  │模型系统核心 │  │移动/桌面App│  │Web 管理平台│
  └────┬─────┘  └───────────┘  └────────────┘  └────────────┘  └───────────┘
       │ 本机显示
       ▼
  ┌──────────┐
  │batana-gui │（嵌入式界面，运行于 pi）
  └──────────┘
```
...
| batana-app | 移动/桌面应用 | Flutter（iOS/Android/macOS/Windows） | 随身入口：录制、外设连接、结果与历史、云同步 |
| batana-gui | 嵌入式界面 | Qt6（C++/QML，嵌入式 Linux） | batana-pi 显示屏的本地界面 |
...
```
 ...
 └──────────┘  控制/OTA └───────┬────────┘    配置下发           └──────────┘
                                ...
                                ▼
 ┌──────────┐   BLE      ┌────────────┐
 │  智能手机 │ ◄────────► │ batana-app │── dart:ffi ──► batana-runtime（pro/standard 档）
 └──────────┘            └────────────┘
```

- **cap → app 或 pi**：BLE GATT 传输 IMU 数据流与设备状态；app/pi 下发采样率、校准、OTA 命令。**cap 为单连接 Peripheral：同一时刻只允许一个 Central 连接**；max 档（pi+cap）时手机不直连 cap，经 pi 的局域网/WebSocket 间接取数。协议规范见 `batana-cap/docs/contracts/ble-protocol.md`。
- **pi 本地闭环**：双目采集 → `batana-runtime` 推理 → 嵌入式 `batana-gui` 显示，离线可用；联网后向 web 同步。
- **app/gui/pi ↔ runtime**：runtime 对外只暴露**稳定 C API**（`runtime-api` 契约，见下表），这是各端与 core 之间唯一允许的耦合面——batana-gui（Qt6/C++）直接链接，batana-app（Flutter）经 dart:ffi 调用。运行时依据已连接外设自动选择能力档位（见 [model-tiers.md](model-tiers.md)）。
- **app/pi → web**：HTTPS 同步会话记录与指标（schema 由 batana-core 定义，见下文契约）；视频与模型工件经预签名 URL 直传对象存储（R2），不进 REST body。
...
| 云端同步 API | batana-web | `docs/contracts/sync-api.md` | app/pi 与 web 之间的 REST 接口；含**视频带外上传**与**模型工件分发**（`/models/manifest`） |
| 模型能力注册表 | batana-core | `runtime/capabilities.h` + `docs/contracts/capabilities.md` | 各档位能力 id 清单，app/gui/web 据此动态渲染功能 |
| runtime-api | batana-core | `runtime/include/batana_runtime.h`（规划） | runtime 稳定 C API；app/gui/pi 与 core 的唯一耦合面，ABI 级版本化 |
...
- **batana-app 只做移动/桌面交互与编排**：不内置评分算法，一律经 dart:ffi 调 runtime；不做服务端逻辑；不做嵌入式界面。
- **batana-gui 只做 pi 显示屏本地界面**：不做移动/桌面端（归 batana-app）；不内置评分算法，直接链接 runtime。
...
| 更新决策 | 版本比较、提示用户、触发更新 | batana-app / batana-pi |
